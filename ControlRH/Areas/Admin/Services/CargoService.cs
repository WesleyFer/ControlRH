...
public class CargoService : ICargoService
{
    ...
    public CargoService(IQueryContext queryContext, IUnitOfWork uow)
    {
    ...
    {
        var query = _queryContext.QueryCargos;
        ...
        {
            var parameter = Expression.Parameter(typeof(Cargo), "c");
            ...
            var genericMethod = method.MakeGenericMethod(typeof(Cargo), property.Type);

            query = (IQueryable<Cargo>)genericMethod.Invoke(null, new object[] { query, lambda });
        }
        ...
        var viewModel = new DynamicTableViewModel
        {
            ...
            Dir = dir,
            AreaName = "Admin",
            ControllerName = nameof(Cargo),
            TextoBotaoAdicionar = "Novo cargo",
            Export = false
        };
    ...
    public async Task<CargoViewModel?> DetailsAsync(Guid id, CancellationToken cancellationToken = default)
    {
    ...
    public async Task InsertAsync(CargoViewModel viewModel, CancellationToken cancellationToken = default)
    {
        ...
        var repositorio = _uow.Repository<Cargo>();
    ...
    public async Task UpdateAsync(Guid id, CargoViewModel viewModel, CancellationToken cancellationToken = default)
    {
        var entidade = await GetByIdAsync(id, cancellationToken);
